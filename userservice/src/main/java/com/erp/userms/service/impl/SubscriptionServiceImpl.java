...
import com.erp.userms.DTO.FeatureDTO;
import com.erp.userms.DTO.ServiceDTO;
import com.erp.userms.DTO.SubscriptionDTO;
import com.erp.userms.entity.*;
import com.erp.userms.repository.*;
...
public class SubscriptionServiceImpl implements SubscriptionInterface {
    ...
    public SubscriptionDTO entityToSubscriptionDto(SubscriptionEntity subscriptionEntity) {
        ...
        for (ServiceEntity serviceEntity : subscriptionEntity.getServiceEntity()) {
            ...
            List<Long> apiPermissionsIds = new ArrayList<>();
            for (ApiPermissionEntity apiPermissionEntity : serviceEntity.getApiPermissionEntity()) {
                apiPermissionsIds.add(apiPermissionEntity.getId()); // Collecting only IDs
            }
            serviceDTO.setApiPermissionsIds(apiPermissionsIds);
...
//    }
    ...
    public ServiceDTO createService(ServiceDTO serviceDTO) {
        ...
        // Handle the mapping between subscription and service
        SubscriptionServicesMapping subscriptionServicesMapping = new SubscriptionServicesMapping();
        subscriptionServicesMapping.setSubscriptionEntity(optionalSubscription);
        subscriptionServicesMapping.setServiceEntity(serviceEntity);
        subscriptionServicesMappingRepo.save(subscriptionServicesMapping);
        ...
        if (serviceDTO.getApiPermissionsIds() != null) {
            List<ApiPermissionEntity> apiPermissions = new ArrayList<>();
            for (Long apiPermissionId : serviceDTO.getApiPermissionsIds()) {
                ApiPermissionEntity apiPermission = apiPermissionRepository.findById(apiPermissionId).orElse(null);
                if (apiPermission != null) {
                    apiPermissions.add(apiPermission);
                } else {
                    LOGGER.warn("API Permission with ID {} not found", apiPermissionId);
                }
            }
            serviceEntity.setApiPermissionEntity(apiPermissions);
        }
    ...
    public ServiceEntity dtoToServiceEntity(ServiceDTO serviceDTO) {
        ...
        if (serviceDTO.getFeature() != null) {
            for (FeatureDTO featureDTO : serviceDTO.getFeature()) {
                featureEntityList.add(dtoToFeatureEntity(featureDTO)); // Assuming you have this method
            }
        }
        service.setFeatureEntity(featureEntityList);

        LOGGER.info("Exiting from the dtoToServiceEntity method in SubscriptionServiceImpl");
        return service;
    }
    public ServiceDTO entityToServiceDTO(ServiceEntity service) {
        ...
        // Assuming subscriptionEntity can have multiple subscriptions
        ...
        // Convert FeatureEntity to FeatureDTO
        List<FeatureDTO> featureDTOList = new ArrayList<>();
        for (FeatureEntity featureEntity : service.getFeatureEntity()) {
            featureDTOList.add(entityToFeatureDTO(featureEntity)); // Assuming you have this method
        }
        serviceDTO.setFeature(featureDTOList);

        // Extract API Permission IDs into the list
        List<Long> apiPermissionsIds = new ArrayList<>();
        for (ApiPermissionEntity apiPermissionEntity : service.getApiPermissionEntity()) {
            apiPermissionsIds.add(apiPermissionEntity.getId()); // Collecting only IDs
        }
        serviceDTO.setApiPermissionsIds(apiPermissionsIds);
...
//        serviceDTO.setFeature(featureDTOList);
//
//        List<ApiPermissionsDTO> apiPermissionsDTOList = new ArrayList<>();
//        for (ApiPermissionEntity apiPermissionEntity : service.getApiPermissionEntity()){
//            apiPermissionsDTOList.add(entityToApiPermissionsDTO(apiPermissionEntity));
//        }
//        serviceDTO.setApiPermissions(apiPermissionsDTOList);
//
//        LOGGER.info("Exiting from the entityToServiceDTO method in SubscriptionServiceImpl");
//        return serviceDTO;
//    }
...
//    }
    ...
    public ApiPermissionsDTO entityToApiPermissionsDTO(ApiPermissionEntity apiPermissionEntity) {
        LOGGER.info("Entering into the entityToServiceDTO method in SubscriptionServiceImpl");
        ApiPermissionsDTO apiPermissionsDTO = new ApiPermissionsDTO();
        apiPermissionsDTO.setId(apiPermissionEntity.getId());
        apiPermissionsDTO.setName(apiPermissionEntity.getName());
        apiPermissionsDTO.setDescription(apiPermissionEntity.getDescription());
        apiPermissionsDTO.setMethod(apiPermissionEntity.getMethod());
        apiPermissionsDTO.setStatus(apiPermissionEntity.getStatus());
        return apiPermissionsDTO;
    }
}
