package com.erp.userms.controller;

import com.DTO.common.PageableResponse;
import com.DTO.common.ResponseBO;
import com.DTO.common.StatusConstant;
import com.erp.userms.DTO.LoginDTO;
import com.erp.userms.DTO.UserDTO;
import com.erp.userms.entity.CustomUserDetails;
import com.erp.userms.security.jwt.JwtResponse;
import com.erp.userms.security.jwt.JwtUtil;
import com.erp.userms.service.EmailService;
import com.erp.userms.service.Interface.ILoginServiceInterface;
import com.erp.userms.service.Interface.RoleServiceInterface;
import com.erp.userms.service.Interface.SubscriptionInterface;
import com.erp.userms.service.Interface.UserServiceInterface;
import com.erp.userms.service.UserService;
import com.erp.userms.service.impl.RegistrationServiceImpl;
...
public class UserController {
    ...
    public ResponseBO registration(@RequestBody UserDTO userDTO) {
        try {
            ...
            if (userServiceInterface.isEmailIdExists(userDTO.getEmailId())) {
                return new ResponseBO(HttpStatus.CONFLICT.value(), HttpStatus.CONFLICT.getReasonPhrase(), null, userDTO.getEmailId() + " " + StatusConstant.EXISTS);
            } else if (userServiceInterface.isMobileNoExists(userDTO.getMobileNo())) {
                return new ResponseBO(HttpStatus.CONFLICT.value(), HttpStatus.CONFLICT.getReasonPhrase(), null, userDTO.getMobileNo() + " " + StatusConstant.EXISTS);
            } else if (userServiceInterface.isGstinExists(userDTO.getOrganization().getGstin())) {
                return new ResponseBO(HttpStatus.CONFLICT.value(), HttpStatus.CONFLICT.getReasonPhrase(), null, userDTO.getOrganization().getGstin() + " " + StatusConstant.EXISTS);
            } else if (userServiceInterface.isPanExists(userDTO.getOrganization().getPan())) {
                return new ResponseBO(HttpStatus.CONFLICT.value(), HttpStatus.CONFLICT.getReasonPhrase(), null, userDTO.getOrganization().getPan() + " " + StatusConstant.EXISTS);
            } else if (userServiceInterface.isTanExists(userDTO.getOrganization().getTan())) {
                return new ResponseBO(HttpStatus.CONFLICT.value(), HttpStatus.CONFLICT.getReasonPhrase(), null, userDTO.getOrganization().getTan() + " " + StatusConstant.EXISTS);
            } else if (userServiceInterface.isCinExists(userDTO.getOrganization().getCin())) {
                return new ResponseBO(HttpStatus.CONFLICT.value(), HttpStatus.CONFLICT.getReasonPhrase(), null, userDTO.getOrganization().getCin() + " " + StatusConstant.EXISTS);
                ...
                if (!subscriptionInterface.isSubscriptionIdExists(userDTO.getSubscriptionId())) {
                    return new ResponseBO(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND.getReasonPhrase(), null, "Subscription Id " + userDTO.getSubscriptionId() + " " + StatusConstant.NOT_FOUND);
                } else if (!roleServiceInterface.isRoleExists(defaultRoleName)) {
                    return new ResponseBO(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND.getReasonPhrase(), null, "Role " + defaultRoleName + " " + StatusConstant.NOT_FOUND);
                }
            }
            ...
            return new ResponseBO(HttpStatus.CREATED.value(), HttpStatus.CREATED.getReasonPhrase(), createdUser, StatusConstant.CREATED);

        } catch (Exception ex) {
            LOGGER.error("Unexpected error during registration: {}", ex.getMessage());
            return new ResponseBO(HttpStatus.INTERNAL_SERVER_ERROR.value(), HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase(), null, "An error occurred: " + ex.getMessage());
        }
    }
    ...
    public ResponseBO updateUser(@RequestBody UserDTO userDTO, @PathVariable Long userId) {
        try {
            ...
            if (updatedUser != null) {
                ...
                return new ResponseBO(HttpStatus.OK.value(),HttpStatus.OK.getReasonPhrase(), updatedUser, StatusConstant.UPDATED);
            } else {
                LOGGER.error("User not found for ID: {}", userDTO.getUserId());
                return new ResponseBO(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND.getReasonPhrase(), null, "User Id" +" "+ userDTO.getUserId() +" "+ StatusConstant.NOT_FOUND);
            }
        } catch (IllegalArgumentException ex) {
            LOGGER.error("Role ID or Subscription ID not found");
            return new ResponseBO(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND.getReasonPhrase(), null, "Role Id:" + userDTO.getRoleId() +" or Subscription Id:" +userDTO.getSubscriptionId() +" "+ StatusConstant.NOT_FOUND);
        } catch (Exception ex) {
            LOGGER.error("An error occurred while processing the request", ex);
            return new ResponseBO(HttpStatus.INTERNAL_SERVER_ERROR.value(),HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase(), null, StatusConstant.INTERNAL_SERVER_ERROR);
        } finally {
    ...
    public ResponseBO getUserById(@PathVariable Long userId) {
        try {
            ...
            if (!userServiceInterface.isUserIdExists(userId)) {
                ...
                return new ResponseBO(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND.getReasonPhrase(), null, "User Id" +" "+ userId +" "+ StatusConstant.NOT_FOUND);
            }

            UserDTO user = userServiceInterface.getUserByUserId(userId);

            LOGGER.info("Exiting the getUserById method successfully");
            return new ResponseBO(HttpStatus.OK.value(),HttpStatus.OK.getReasonPhrase(), user, StatusConstant.GET);
        } catch (Exception ex) {
            LOGGER.error("An error occurred while processing the request", ex);
            return new ResponseBO(HttpStatus.INTERNAL_SERVER_ERROR.value(),HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase(), null, StatusConstant.INTERNAL_SERVER_ERROR);
        }
    ...
    public PageableResponse<?> getAllUsers(
        ...
        try {
            ...
            if (userDTOs.getData() == null || !(userDTOs.getData() instanceof List) || ((List<?>) userDTOs.getData()).isEmpty()) {
                ...
                return new PageableResponse<>(HttpStatus.NO_CONTENT.value(), HttpStatus.NO_CONTENT.getReasonPhrase(), 0, 0, null, StatusConstant.NO_CONTENT, 0, 0);
            }

            userDTOs.setCode(HttpStatus.OK.value());
            ...
            return new PageableResponse<>(HttpStatus.INTERNAL_SERVER_ERROR.value(), HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase(), 0, 0, null, StatusConstant.INTERNAL_SERVER_ERROR, 0, 0);
        }
    }
    ...
    public ResponseBO getUserByUsername(@PathVariable String username) {
        try {
            ...
            if (userDetails == null) {
                ...
                return new ResponseBO(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND.getReasonPhrase(), null, username+" "+StatusConstant.NOT_FOUND);
            }
            return new ResponseBO(HttpStatus.OK.value(),HttpStatus.OK.getReasonPhrase(), userDetails, StatusConstant.GET);
        }   catch (Exception ex) {
    ...
    public ResponseBO getUsersByRoleId(@PathVariable Long roleId) {
        try {
            ...
            if (!roleServiceInterface.isRoleIdExists(roleId)){
                return new ResponseBO(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND.getReasonPhrase(), null, "Role ID" +" "+ roleId +" "+ StatusConstant.NOT_FOUND);
            }
            ...
            if (user.isEmpty()) {
                return new ResponseBO(HttpStatus.NO_CONTENT.value(), HttpStatus.NO_CONTENT.getReasonPhrase(), null, StatusConstant.NO_CONTENT);

            }
            return new ResponseBO(HttpStatus.OK.value(),HttpStatus.OK.getReasonPhrase(), user, user.size() + " " + StatusConstant.GET_LIST);
        } catch (Exception ex) {
            LOGGER.error("An error occurred while processing the request", ex);
            return new ResponseBO(HttpStatus.INTERNAL_SERVER_ERROR.value(),HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase(), null, StatusConstant.INTERNAL_SERVER_ERROR);
        }
    ...
    public ResponseBO deleteUser(@PathVariable Long userId) {
        try {
            ...
            return new ResponseBO(HttpStatus.OK.value(),HttpStatus.OK.getReasonPhrase(), null, StatusConstant.DELETED);
    ...
    public JwtResponse login(@RequestBody LoginDTO loginDTO) {
        try {
            ...
            LOGGER.error("An error occurred while processing the request", ex);
            return new JwtResponse(HttpStatus.INTERNAL_SERVER_ERROR.value(),HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase(), null, null, StatusConstant.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseBO getUserDetails(@PathVariable String username, @RequestHeader("Authorization") String token) {
        try {
            ...
            if (user.getOrganization().getOrganizationSubscription().getSubscription() == null) {
                return new ResponseBO(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND.getReasonPhrase(), user, "User has not subscribed yet");
            }}
    ...
    public ResponseBO logout(@PathVariable("username") String username) {
        try {
            if (iLoginServiceInterface.isUsernameExists(username)){
                ...
                return new ResponseBO(HttpStatus.OK.value(),HttpStatus.OK.getReasonPhrase(), null, "Logout Successfully");
            } else {
                LOGGER.info("Username Incorrect");
                return new ResponseBO(HttpStatus.UNAUTHORIZED.value(),HttpStatus.UNAUTHORIZED.getReasonPhrase(), null, username + "Incorrect");
            }
        }   catch (Exception ex) {
            LOGGER.error(ExceptionUtils.getStackTrace(ex));
            return new ResponseBO(HttpStatus.INTERNAL_SERVER_ERROR.value(),HttpStatus.INTERNAL_SERVER_ERROR.getReasonPhrase(), null, StatusConstant.INTERNAL_SERVER_ERROR);
    ...
    public ResponseBO resetPassword(@RequestBody LoginDTO loginDTO, @PathVariable String username) {
        try {
            ...
            if (Objects.equals(reset, "active")) {
                return new ResponseBO(HttpStatus.OK.value(),HttpStatus.OK.getReasonPhrase(), null, "Password reset successfully");
            }if (Objects.equals(reset, "inActive")) {
                return new ResponseBO(HttpStatus.OK.value(),HttpStatus.OK.getReasonPhrase(), null, "Password not reset");
            } else {
                return new ResponseBO(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND.getReasonPhrase(), null, username +" "+ StatusConstant.NOT_FOUND);
            }
